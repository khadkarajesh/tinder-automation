...
if __name__ == "__main__":
    ...
    for person in recommendations['results']:
        #print(superlike(person['_id']))
        if like(person['_id'])['likes_remaining'] ==  0:
            break
        print('liked of name {} '.format(person['name']))
    updates = get_updates()
    for match in updates['matches']:
        send_msg(match['_id'], "Hey! How you doing?")
    print('all done')
